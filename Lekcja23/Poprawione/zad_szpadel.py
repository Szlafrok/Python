...
class Gracz(Postac):
    ...
    def walka(self, przeciwnik):
        ...
        while czy_walka:
            ...
            akcja = input("Wybierz: atak/ucieczka/jedz: ")
            if akcja == "atak":
                ...
                if self.liczba_uleczen == 0: # Dobry dodatek
                    print("Niestety liczba twoich uleczeń się skończyła, więc pomijasz turę")
                    przeciwnik.atakuj(self) # To również dobry dodatek!
                    continue # pominięcie dalszej części! Super!

                print(f"Możesz jeszcze zjeść tylko {self.liczba_uleczen} razy")
                czy_zjesc = input("Czy jesteś pewien tak/nie: ").lower()
                if czy_zjesc == "tak":
                    self.liczba_uleczen -= 1
                    self.zjedz()
                elif czy_zjesc == "nie":
                    print("Nie jesz i kończysz turę")
                else:
                    print("Zły wybór, tracisz kolejkę")
                    przeciwnik.atakuj(self)
...
# Bardzo ładnie, brak zastrzeżeń :)

# 3/3 +1 bonusu za logikę rozgrywki